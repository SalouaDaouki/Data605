...
knitr::include_graphics("/Users/salouadaouki/Desktop/Data605/ANOVA_plot.pdf")
knitr::include_graphics("/Users/salouadaouki/Desktop/Data605/scatterplot_matrix.pdf")
knitr::include_graphics("/Users/salouadaouki/Desktop/Data605/residual_plots.png")
```
...
pdf("ANOVA_plot.pdf", width = 10, height = 8)
...
![ANOVA Plot](/Users/salouadaouki/Desktop/Data605/ANOVA_plot.pdf)
...
The p-value, 2.2e-16, is very small which indicates strong evidence against the null hypothesis (that the correlation between these variables is 0). Therefore, we reject the null hypothesis, suggesting that the correlation between LotArea and SalePrice is statistically significant.
The 99% Confidence Interval for the correlation coefficient: 0.2154574 to 0.3109369, emphasizes that the correlation coefficient is not 0. Meaning that the true correlation coefficient falls within the indicated interval.
The "sample estimates", estimates the correlation coefficient which I calculated before; 0.2638434.
...
It seems that the histogram and the PDF closely align, that suggests a good fit.
After visualizing the histogram of the shifted LotArea, we can plot a histogram of the original LotArea and compare it to the shifted one.
...
![Scatter Plot MAtrix](/Users/salouadaouki/Desktop/Data605/scatterplot_matrix.pdf)
...
# Split numerical data into subsets (excluding SalePrice)
subsets <- split(numerical_data[, -which(names(numerical_data) == "SalePrice")], 1:num_subset)

# Create a list to store correlation matrices
correlation_matrices <- vector("list", length = num_subset)

# Calculate correlation matrices for each subset
for (i in 1:num_subset) {
  # Combine SalePrice with the subset
  subset_with_saleprice <- cbind(subsets[[i]], SalePrice = numerical_data$SalePrice)
  # Calculate correlation matrix
  correlation_matrices[[i]] <- cor(subset_with_saleprice)
}

# Plot heatmaps for each correlation matrix
for (i in 1:num_subset) {
  # Convert the correlation matrix to long format
  correlation_long <- melt(correlation_matrices[[i]])
...
```{r}
# Function to calculate correlation matrix and plot heatmap for a subset
plot_correlation_heatmap <- function(subset_data, subset_name) {
  # Calculate correlation matrix
  correlation_matrix <- cor(subset_data)
  
  # Find variables strongly correlated with SalePrice
  strong_correlation_with_saleprice <- correlation_matrix["SalePrice", ] >= strong_correlation_threshold
  
  # Convert correlation matrix to long format
  correlation_long <- melt(correlation_matrix_New)
  ...
  ggplot(correlation_long, aes(Var1, Var2, fill = value)) +
    ...
    theme(axis.text.x = element_text(angle = 45, vjust = 1, size = 12, hjust = 1)) +
    coord_fixed() +
    ggtitle(paste("Correlation Heatmap -", subset_name)) +  # Add subset name to title
    theme(plot.title = element_text(hjust = 0.5)) +  # Center the title
    geom_tile(data = correlation_long[strong_correlation_with_saleprice, ],
              aes(Var1, Var2), fill = "black", color = "black")  # Highlight strong correlations with SalePrice
}

# Set threshold for strong correlation with SalePrice
strong_correlation_threshold <- 0.5

# Split numerical variables into subsets
num_subset <- 10  # Adjust as needed
subset_size <- ceiling((length(numerical_data) - 1) / num_subset)  # -1 to exclude SalePrice
subset_names <- paste0("Subset", 1:num_subset)

# Create subsets
subsets <- lapply(1:num_subset, function(i) {
  start_index <- (i - 1) * subset_size + 1
  end_index <- min(i * subset_size, length(numerical_data))
  subset_data <- numerical_data[, start_index:end_index]  # Subset without SalePrice
  subset_data$SalePrice <- numerical_data$SalePrice  # Add SalePrice as the last column
  return(subset_data)
})

# Plot correlation heatmaps for each subset
plots <- lapply(1:num_subset, function(i) {
  plot_correlation_heatmap(subsets[[i]], subset_names[i])
})
...
It is still hard to read the heat map even though the data was split into 5 then 10 subsets. Let's run the full linear model using all numerical variables as predictors for SalePrice.
...
Based on the summary of the full regression model, the predictors that explain the Sale price are 'MSSubClass' (The building class), 'LotArea' (Lot size in square feet), 'OverallQual' (Overall material and finish quality), 'OverallCond' (Overall condition rating), 
YearBuilt' (Original construction date), 'MasVnrArea' (Masonry veneer area in square feet), 'BsmtFinSF1' (Type 1 finished square feet), 'X1stFlrSF' (First Floor square feet), 'X2ndFlrSF' (Second floor square feet), 'BsmtFullBath' (Basement full bathrooms), 'BedroomAbvGr' (Number of bedrooms above ground), 'TotRmsAbvGrd' (Total rooms above grade (does not include bathrooms)), 'GarageCars' (Size of garage in car capacity), 'WoodDeckSF' (Wood deck area in square feet), and  'ScreenPorch' (Screen porch area in square feet).

Now, let’s perform the backward process elimination:


```{r}
# Assuming model is your full linear regression model
